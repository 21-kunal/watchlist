...
select_upcoming_movies = """SELECT * 
                            FROM movies 
                            WHERE release_timestamp > ?;"""

select_watched_movies =  """SELECT * 
                            FROM watched 
                            WHERE user = ?;"""

insert_watched_movies = """INSERT INTO watched
                            (user,title)
                            VALUES (?,?);"""
...
delete_movie_from_movies =  """DELETE FROM movies
                                WHERE title = ?;"""

delete_movie_from_watched = """DELETE FROM watched
                                WHERE title = ?;"""
...
def delete_movie(title):
    with connection:
        connection.execute(delete_movie_from_movies,(title,))
        connection.execute(delete_movie_from_watched,(title,))